...
def get_data(statistic, return_model=True, select_coordinates={}, slice_coordinates={}, select_indices=[]):
...
def read_greedy_bins():
    ...
    bins_fn = Path(bins_dir) / 'fixed_blcdm_marginalised_hod_v2.json'
    with open(bins_fn, 'r') as f:
        selection_history = json.load(f)['selection_history']
...
print(selection_history[0])
print(selection_history[1])
print(selection_history[-1])
...
#surviving_bin_idx = get_filtered_bin_indices(coords=coords, select_coordinates=select_mocks)
...
# for q in [0, 1, 3, 4]:
#     select_coordinates = {'multipoles': [0], 'cosmo_idx': 0, 'hod_idx': 30,
...
# slice_coordinates = {}
# sep, data_y, error, model = get_data(statistic, slice_coordinates=slice_coordinates)
